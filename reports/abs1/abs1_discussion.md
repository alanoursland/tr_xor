...
Even when minimal re-orientation was needed, performance was still dictated by scale. In the `Large` initialization runs, trials requiring the smallest angle changes were still extremely slow, averaging 628.8 epochs to converge. When the magnitude delta dominates the angle delta, we see a clear linear relationship in training time. But, these two factors are joint variables. We can't really draw much of a conclusion from this data. More controlled and rigorous experimentation is needed to discover any relationship, if it exists at all.
