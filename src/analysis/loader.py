...
def load_all_run_results(results_dir: Path, config: ExperimentConfig) -> List[Dict[str, Any]]:
    ...
    run_dirs = sorted([d for d in runs_dir.iterdir() if d.is_dir() and d.name.isdigit()], key=lambda d: int(d.name))
...
def load_run_checkpoints(checkpoints_dir: Path) -> List[Dict[str, Any]]:
    ...
    checkpoint_files = sorted(
        [f for f in checkpoints_dir.iterdir() if f.is_file() and f.name.endswith(".pt")],
