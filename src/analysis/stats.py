...
def compute_summary_statistics(metrics: Dict[str, List[float]]) -> Dict[str, Dict[str, float]]:
    ...
    for metric_name, values in metrics.items():
        if not values:
            summary[metric_name] = {"count": 0, "mean": 0.0, "std": 0.0, "min": 0.0, "max": 0.0, "median": 0.0}
            continue
...
def compute_distribution_statistics(metrics: Dict[str, List[float]], config: ExperimentConfig) -> Dict[str, Any]:
    ...
    if metrics["final_losses"]:
        ...
        distributions["loss_distribution"] = {
            ...
            "convergence_rate": sum(1 for loss in metrics["final_losses"] if loss < loss_change_threshold)
            / len(metrics["final_losses"]),
    ...
    if metrics["training_times"]:
        ...
        distributions["training_time_distribution"] = {
            ...
            "fast_runs_rate": sum(
                1 for t in metrics["training_times"] if t < np.percentile(metrics["training_times"], 25)
            )
            / len(metrics["training_times"]),
        }
...
def extract_run_metrics(run_results: List[Dict[str, Any]]) -> Dict[str, List[float]]:
    ...
    for result in run_results:
        ...
        if "training_efficiency" in result:
            metrics["convergence_epochs"].append(result["training_efficiency"].get("convergence_epoch", 0))
        else:
        ...
        if "model_analysis" in result:
            ...
            if "weight_statistics" in model_analysis:
                metrics["weight_norms"].append(model_analysis["weight_statistics"].get("norm", 0.0))
            else:
                metrics["weight_norms"].append(0.0)

            if "bias_statistics" in model_analysis:
                metrics["bias_norms"].append(model_analysis["bias_statistics"].get("norm", 0.0))
            else:
...
def compute_consistency_score(run_results: List[Dict[str, Any]]) -> float:
    ...
    losses = [
        r.get("final_loss", float("inf")) for r in run_results if r.get("final_loss", float("inf")) != float("inf")
    ]
...
def compute_model_statistics(run_results: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    if potential_issues:
        ...
        model_stats["issue_rate"] = len(
            [r for r in run_results if r.get("model_analysis", {}).get("potential_issues", [])]
        ) / len(run_results)
