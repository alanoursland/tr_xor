...
from models import (
    MLP, XORNet, MinimalXORNet, SymmetricXORNet, DualPathXORNet,
    ActivationType, InitializationType, ActivationRegistry,
    create_xor_model, create_parity_model, create_custom_model
)
from data import (
    generate_xor_data, generate_parity_data, sample_input_space,
    NormalizationType, GridType, normalize_dataset, create_training_batches
)
from utils import (
    set_global_random_seeds, setup_experiment_logging, save_model_with_metadata,
    load_model_with_validation, compute_point_to_hyperplane_distance,
...
def quick_xor_experiment(activation: str = "relu", hidden_units: int = 2, 
                        num_runs: int = 5, analyze_results: bool = True) -> Dict[str, Any]:
    """
...
def compare_activations_on_xor(activations: List[str] = None, num_runs: int = 10,
...
def parity_scaling_study(max_bits: int = 5, activations: List[str] = None,
...
def prototype_surface_investigation(model_path: Path, data_type: str = "xor",
...
def ablation_study(base_config: str, components_to_ablate: List[str],
                  num_runs: int = 5) -> Dict[str, Any]:
    """
...
def create_and_train_xor_model(activation: str = "relu", hidden_units: int = 2,
...
def train_model_with_config(model: nn.Module, config_name: str, 
                          data: Optional[Tuple[torch.Tensor, torch.Tensor]] = None) -> Tuple[nn.Module, Dict[str, Any]]:
    """
...
def fine_tune_model(base_model: nn.Module, new_data: Tuple[torch.Tensor, torch.Tensor],
                   epochs: int = 100, learning_rate: float = 0.001) -> Tuple[nn.Module, Dict[str, Any]]:
    """
...
def get_parity_data(n_bits: int, normalized: bool = True, 
...
def create_visualization_grid(bounds: Tuple[Tuple[float, float], ...] = None,
...
def augment_xor_data(noise_std: float = 0.1, num_samples: int = 100,
                    rotation_angles: List[float] = None) -> Tuple[torch.Tensor, torch.Tensor]:
    """
...
def preprocess_data(x: torch.Tensor, method: str = "standardize",
                   fit_params: Optional[Dict[str, torch.Tensor]] = None) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    """
...
def analyze_trained_model(model: nn.Module, data: Tuple[torch.Tensor, torch.Tensor],
...
def plot_model_hyperplanes(model: nn.Module, data: Tuple[torch.Tensor, torch.Tensor],
                          save_path: Optional[Path] = None, style: str = "default") -> Any:
    """
...
def visualize_prototype_regions(model: nn.Module, bounds: Tuple[Tuple[float, float], ...] = None,
                              resolution: int = 100, save_path: Optional[Path] = None) -> Any:
    """
...
def plot_training_history(training_logs: List[Dict[str, Any]], 
                         metrics: List[str] = None, save_path: Optional[Path] = None) -> Any:
    """
...
def compare_models_visually(models_dict: Dict[str, nn.Module], 
...
def create_quick_config(model_type: str = "xor", activation: str = "relu",
                       ...
                       learning_rate: float = 0.01) -> ExperimentConfig:
    """
...
def validate_setup(config: ExperimentConfig, data: Tuple[torch.Tensor, torch.Tensor],
                  model: nn.Module) -> Tuple[bool, List[str]]:
    """
...
def test_separation_order_theory(activation: str, max_bits: int = 4,
                                num_runs: int = 5) -> Dict[str, Any]:
    """
...
def analyze_prototype_surface_consistency(models: List[nn.Module], 
                                        data: Tuple[torch.Tensor, torch.Tensor]) -> Dict[str, Any]:
    """
...
def investigate_distance_based_classification(model: nn.Module, 
...
def batch_train_models(configs: List[ExperimentConfig], parallel: bool = True,
...
def parameter_sweep_helper(base_config: ExperimentConfig, 
                         param_ranges: Dict[str, List[Any]],
                         num_runs_per_config: int = 5) -> Dict[str, Any]:
    """
...
def automated_analysis_pipeline(experiment_dir: Path, 
...
def debug_training_failure(model: nn.Module, data: Tuple[torch.Tensor, torch.Tensor],
...
def profile_training_performance(config: ExperimentConfig, 
...
def create_reproducibility_package(experiment_dir: Path, 
                                  output_path: Path) -> None:
    """
...
def show_prototype_surface_concept(model: nn.Module, 
